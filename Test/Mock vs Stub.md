...
> GTest (Google Test) 라이브러리를 사용한다고 해서 **모든 테스트가 Mock Test는 아님.**

GMock은 GTest에 포함된 모킹 프레임워크이며, `ON_CALL()`과 `EXPECT_CALL()` 모두 사용할 수 있음.  

-> 그러나 **사용 방식에 따라 Stub인지, Mock인지**가 결정됨
